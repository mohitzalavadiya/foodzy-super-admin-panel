...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // 2. We need to create the profile entry as super_admin
        // Note: In production, you would use a trigger or a secure edge function
        // For development, we'll try to insert directly if RLS allows
        ...
        if (profileError) {
          toast.error("User created, but failed to assign admin role. Please use the SQL editor for the first user.")
    ...
    <div className="relative flex min-h-screen w-full items-center justify-center p-4 auth-gradient overflow-hidden">
      ...
      <div className="relative w-full max-w-[440px] animate-in fade-in slide-in-from-bottom-8 duration-700">
        <div className="flex flex-col items-center mb-10">
          <div className="h-16 w-16 rounded-2xl bg-primary flex items-center justify-center shadow-2xl shadow-primary/20 mb-6 font-black italic text-2xl text-white">
            AI
          </div>
          <h1 className="text-4xl font-black tracking-tight text-zinc-900 dark:text-white uppercase leading-none text-center">
            Create <span className="text-primary italic">Admin</span>
          </h1>
          <p className="mt-3 text-zinc-500 dark:text-zinc-400 font-medium text-sm">
            Join the Central Intelligence platform
          </p>
        </div>
          ...
          <form onSubmit={handleSignup}>
            <CardContent className="grid gap-5 py-6">
              <div className="grid gap-2">
                <Label htmlFor="fullName" className="text-xs font-bold uppercase tracking-wider text-zinc-500 ml-1">Full Name</Label>
                <Input
                  id="fullName"
                  type="text"
              ...
              <div className="grid gap-2">
                <Label htmlFor="password" className="text-xs font-bold uppercase tracking-wider text-zinc-500 ml-1">Secure Password</Label>
                <Input
                  id="password"
                  type="password"
                  placeholder="••••••••"
            ...
            <CardFooter className="flex flex-col gap-6 pb-8">
              <Button className="pill-button w-full h-14 text-base shadow-xl shadow-primary/20" type="submit" disabled={loading}>
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Deploy Admin Credentials"}
              </Button>
              
              <div className="flex items-center justify-center gap-2 text-xs font-bold text-zinc-400 uppercase tracking-widest leading-none">
        ...
        <p className="mt-8 text-center text-xs font-bold text-zinc-400 uppercase tracking-widest">
          Secure Deployment • Platform Protocol 2.4
        </p>
      </div>
    </div>
  )
